...
class Rep_desglose_guia extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('rep_desglose_guia_model');
	...
	public function index($opc_id){
		...
		if($_POST){
			...
			$cns_guias=$this->rep_desglose_guia_model->lista_guias_buscador($f1,$f2,$emisor,$txt);
		}else{
			$txt='';
			$emisor='6';
			$f1= date('Y-m-d');
			...
			$this->load->view('reportes/rep_desglose_guia',$data);
    ...
    public function excel($opc_id,$fec1,$fec2){
    	...
    	$titulo='Reporte Desglose Notas de Credito ';
    	$file="rep_desglose_guia".date('Ymd');
