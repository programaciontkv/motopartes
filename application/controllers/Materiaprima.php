...
class Materiaprima extends CI_Controller {
	...
	public function index($opc_id){
		...
		if($_POST){
			...
			$cns_productos=$this->materiaprima_model->lista_productos_buscador($text,$txt_est);
		}else{
			$text= '';
			$txt_est="1";
			$est="1";
		...
		$this->load->view('materiaprima/lista',$data);
		$modulo=array('modulo'=>'materiaprima');
	...
	public function nuevo($opc_id){
		if($this->permisos->rop_insertar){
			...
			$data=array(
						...
						'familias'=>$this->tipo_model->lista_familias('1'),
						'tipos'=>'',
						...
						'action'=>base_url().'materiaprima/guardar/'.$opc_id,
						'opc_id'=>$opc_id	
						);
			$this->load->view('materiaprima/form',$data);
			$modulo=array('modulo'=>'materiaprima');
			$this->load->view('layout/footer',$modulo);
		}else{
	...
	public function guardar($opc_id){
		...
		if($this->form_validation->run()){
			...
			if($this->materiaprima_model->insert($data)){
				$data_aud=array(
				...
				redirect(base_url().'materiaprima/'.$opc_id);
			
			}else{
				$this->session->set_flashdata('error','No se pudo guardar');
				redirect(base_url().'materiaprima/nuevo/'.$opc_id);
			}
		}else{
			$this->nuevo($opc_id);
		}	
	...
	public function editar($id,$opc_id){
		if($this->permisos->rop_actualizar){
			$rst=$this->materiaprima_model->lista_un_producto($id);
			$data=array(
						'estados'=>$this->estado_model->lista_estados_modulo($this->permisos->opc_id),
						'dec'=>$this->configuracion_model->lista_una_configuracion('2'),
						'dcc'=>$this->configuracion_model->lista_una_configuracion('1'),
						...
						'producto'=>$this->materiaprima_model->lista_un_producto($id),
						'action'=>base_url().'materiaprima/actualizar/'.$opc_id,
						'opc_id'=>$opc_id
						);
	...
	public function actualizar($opc_id){
		...
		$producto_act=$this->materiaprima_model->lista_un_producto($id);
		...
		if($this->form_validation->run()){
			...
			if($this->materiaprima_model->update($id,$data)){
				$data_aud=array(
								'usu_id'=>$this->session->userdata('s_idusuario'),
								'adt_date'=>date('Y-m-d'),
								'adt_hour'=>date('H:i'),
				...
				redirect(base_url().'materiaprima/editar'.$id.'/'.$opc_id);
			}
		}else{
	...
	public function visualizar($id){
		if($this->permisos->rop_reporte){
			...
			$this->load->view('materiaprima/visualizar',$data);
		}else{
			redirect(base_url().'inicio');
		}	
	}
	...
	public function eliminar($id,$nombre){
		if($this->permisos->rop_eliminar){
			if($this->materiaprima_model->delete($id)){
				$data_aud=array(
								'usu_id'=>$this->session->userdata('s_idusuario'),
								'adt_date'=>date('Y-m-d'),
								'adt_hour'=>date('H:i'),
    ...
    public function ver_img($id){
		if($this->permisos->rop_reporte){
			...
			$cns=$this->materiaprima_model->lista_un_producto($id);
