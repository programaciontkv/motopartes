...
class pdf_vencidos_cxc_model extends CI_Model {
	...
	public function buscar_documentos_vencidos($act, $fec1, $fec2, $txt,$emp){
		$query ="SELECT c.fac_id,c.fac_nombre,c.fac_identificacion,c.fac_fecha_emision, c.fac_total_valor,c.fac_numero,c.cli_id FROM  erp_factura c, erp_pagos_factura p where c.fac_id=p.com_id  and p.pag_fecha_v <= '$act' and (c.fac_estado=4 or c.fac_estado=6) and c.fac_fecha_emision between '$fec1' and '$fec2' $txt and c.emp_id=$emp and ((select sum(pag_cant) from erp_pagos_factura p1 where p1.com_id=c.fac_id and pag_estado=1)>(select sum(ct.cta_monto) from erp_ctasxcobrar ct where c.fac_id=ct.com_id and cta_estado=1) or not exists(select * from erp_ctasxcobrar ct where c.fac_id=ct.com_id and cta_estado=1)) group by c.fac_id,c.fac_nombre,c.fac_identificacion,c.fac_fecha_emision, c.fac_total_valor,c.fac_numero,c.cli_id order by c.fac_nombre,c.fac_identificacion,c.fac_numero";
