...
class Ctasxcobrar_pagos_model extends CI_Model {
	...
	public function lista_factura_buscador($text,$f1,$f2,$emp_id){
		$this->db->select("f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,(select sum(cta_monto) from erp_ctasxcobrar where com_id=f.fac_id and cta_fecha_pago<='$f2' and cta_estado=1) as pago,saldo,fac_estado");
		...
		$this->db->group_by('f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,saldo,fac_estado');
		$this->db->order_by('f.fac_nombre','asc');
		$this->db->order_by('f.fac_identificacion','asc');
	...
	public function lista_vencer_vencido($text,$f1,$f2,$emp_id){
		$query="select f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,fac_estado, (select sum(cta_monto) from erp_ctasxcobrar where com_id=f.fac_id and cta_fecha_pago<='$f2' and cta_estado=1) as pago 
from erp_factura f, pagosxfactura p 
where f.fac_id=p.fac_id and (f.fac_numero like '%$text%' or fac_nombre like '%$text%' or fac_identificacion like '%$text%') and f.fac_fecha_emision between '$f1' and '$f2' and fac_estado!=3 and emp_id=$emp_id and (p.fac_total_valor>(select sum(ct.cta_monto) from erp_ctasxcobrar ct where f.fac_id=ct.com_id and ct.cta_estado='1' and cta_fecha_pago between '$f1' and '$f2') or not exists(select * from erp_ctasxcobrar ct where f.fac_id=ct.com_id and ct.cta_estado='1' and cta_fecha_pago between '$f1' and '$f2')) and pag_fecha_v > '$f2'
group by f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,fac_estado, saldo, pago
union 
select f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,fac_estado, (select sum(cta_monto) from erp_ctasxcobrar where com_id=f.fac_id and cta_fecha_pago<='$f2' and cta_estado=1) as pago
from erp_factura f, pagosxfactura p
...
order by fac_nombre, fac_numero ";
	...
	public function lista_vencer($text,$f1,$f2,$emp_id){
		$this->db->select("f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,fac_estado, (select sum(cta_monto) from erp_ctasxcobrar where com_id=f.fac_id and cta_fecha_pago<='$f2' and cta_estado=1) as pago");
		$this->db->from('erp_factura f');
		$this->db->join('pagosxfactura p','f.fac_id=p.fac_id');
		...
		$this->db->group_by('f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,pago,saldo,fac_estado');
		$this->db->order_by('f.fac_nombre','asc');
		$this->db->order_by('fac_numero','asc');
		$resultado=$this->db->get();
		return $resultado->result();
	...
	public function lista_vencido($text,$f1,$f2,$emp_id){
		$this->db->select("f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,saldo,fac_estado, (select sum(cta_monto) from erp_ctasxcobrar where com_id=f.fac_id and cta_fecha_pago<='$f2' and cta_estado=1) as pago");
		$this->db->from('erp_factura f');
		$this->db->join('pagosxfactura p','f.fac_id=p.fac_id');
	...
	public function lista_pagado($text,$f1,$f2,$emp_id){
		...
		$this->db->from('erp_factura f');
		$this->db->join('pagosxfactura p','f.fac_id=p.fac_id');
		$this->db->where("(f.fac_numero like '%$text%' or fac_nombre like '%$text%' or fac_identificacion like '%$text%') and f.fac_fecha_emision between '$f1' and '$f2' and fac_estado!=3 and emp_id=$emp_id and (select sum(cta_monto) from erp_ctasxcobrar where com_id=f.fac_id and cta_fecha_pago<='$f2' and cta_estado=1)=p.fac_total_valor", null);
		$this->db->group_by('f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,pago,saldo,fac_estado');
		$this->db->order_by('f.fac_nombre','asc');
		$this->db->order_by('fac_numero','asc');
		$resultado=$this->db->get();
	...
	public function lista_saldo_factura($id){
		$this->db->select('f.fac_id,f.fac_fecha_emision, pag_fecha_v, f.fac_numero, f.fac_nombre, f.fac_identificacion,p.fac_total_valor,pago,saldo,fac_estado');
		$this->db->from('erp_factura f');
		$this->db->join('pagosxfactura p','f.fac_id=p.fac_id');
