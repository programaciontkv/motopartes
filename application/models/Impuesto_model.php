...
class Impuesto_model extends CI_Model {
	...
	public function lista_un_impuesto($id){
		...
		// $resultado=$this->db->get();
		$query ="select i.*,p.pln_id,p.pln_codigo, p.pln_descripcion,e.est_descripcion from porcentages_retencion i, erp_estados e, erp_plan_cuentas p where e.est_id=i.por_estado and p.pln_id=i.cta_id and i.por_id='$id'
			union 
			select i.*,'0','','',e.est_descripcion from porcentages_retencion i, erp_estados e where e.est_id=i.por_estado and i.cta_id=0 and i.por_id='$id'";
        $resultado=$this->db->query($query);
