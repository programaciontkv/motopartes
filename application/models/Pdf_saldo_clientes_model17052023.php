...
class pdf_saldo_clientes_model extends CI_Model {
	...
	public function lista_documentos_ctas($nm,$emp){
		$query ="select f.fac_identificacion, f.fac_nombre, c.pln_id from erp_factura f, erp_ctasxcobrar c where c.com_id=f.fac_id  $nm and f.emp_id=$emp group by f.fac_nombre,f.fac_identificacion,c.pln_id
                     union
                     select f.fac_identificacion, f.fac_nombre,'0' as pln_id from erp_factura f where not exists(select * from erp_ctasxcobrar c where c.com_id=f.fac_id) $nm and f.emp_id=$emp group by f.fac_nombre,f.fac_identificacion order by fac_nombre, fac_identificacion,pln_id desc";
	...
	public function suma_documentos_cliente($id,$fec,$emp){
		$query ="select (select sum(p.fac_total_valor) from pagosxfactura p, erp_factura f where f.fac_id=p.fac_id and f.fac_identificacion ='$id' and (f.fac_estado=6 or fac_estado=4) and f.emp_id=$emp and f.fac_fecha_emision<='$fec') as fac_total_valor,
